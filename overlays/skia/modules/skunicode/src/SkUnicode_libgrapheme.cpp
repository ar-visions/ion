...
public:
    ...
    /*
    void grapheme_bidi_reorderVisual(const uint8_t* runLevels,
                                    int levelsCount,
                                    int32_t* logicalFromVisual) {
        // Convert runLevels from uint8_t to uint_least32_t
        std::vector<uint_least32_t> linedata(levelsCount);
        std::vector<uint_least32_t> runLevels_32(levelsCount);
        for (int i = 0; i < levelsCount; i++) {
            runLevels_32[i] = (uint_least32_t)(runLevels[i]);
        }
        /// grapheme expects resolved array, not an index of logicalFromVisual

        size_t outputSize = grapheme_bidirectional_reorder_line(
            graphemeRunLevels.data(), runLevels_32.data(), levelsCount,
            reinterpret_cast<uint_least32_t*>(logicalFromVisual), levelsCount);

        // Check if the outputSize is equal to levelsCount to ensure successful reordering
        assert (outputSize == static_cast<size_t>(levelsCount));
    }*/
    ...
    void reorderVisual(const BidiLevel runLevels[],
        ...
        // todo:
        // this needs to be done with grapheme!
        // why use icu when we dont build with it?
        // can only imagine they couldnt figure this one out (i cant either)
        //fSkUnicode_IcuBidi::bidi_reorderVisual(runLevels, levelsCount, logicalFromVisual);
    }
private:
