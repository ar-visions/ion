...
struct Render {
    Device            *device;
    std::queue<memory *> sequence;     /// sequences of identical pipeline data for multiple render passes
...
PipelineData::PipelineData(Device *device, UniformData &ubo, VertexData &vbo, IndexData &ibo,
                Assets &assets, size_t vsize, rgba clr, str shader, VkStateFn vk_state) :
                    PipelineData(
                        new PipelineMemory { device, ubo, vbo, ibo, assets, vsize, clr, shader, vk_state }) {
    m = (PipelineMemory *)mem->origin;
}
...
PipelineMemory::PipelineMemory(Device   *device,  UniformData &ubo,
    ...
    */
    ...
    VkResult res0 = vkCreateGraphicsPipelines(*device, VK_NULL_HANDLE, 1, &pi, nullptr, &pipeline);
...
void Render::present() {
    ...
    vkWaitForFences(device, 1, &fence_active[cframe], VK_TRUE, UINT64_MAX); /// this is a nullptr
    ...
    while (sequence.size()) {
        memory      *pmem = sequence.front(); /// this one is a nullptr_t
        PipelineMemory &m = *pmem->data<PipelineMemory>(0); /// not initialized as zero it seems.
        sequence.pop();
        //pmem->drop();
