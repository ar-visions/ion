...
struct gfx_memory {
    ...
    ~gfx_memory() {
        if (ctx)        vkg_drop        (ctx);
        if (vg_device)  vkg_device_drop(vg_device);
        if (vg_surface) vkg_surface_drop(vg_surface);
    }
};
...
gfx::gfx(ion::window &win) : gfx(mx::alloc<gfx>()) { /// this allocates both gfx_memory and cbase::cdata memory (cbase has data type aliased at cbase::DC)
    ...
    g->vg_device        = vkg_device_create_multisample(win->vke_device, VK_SAMPLE_COUNT_8_BIT);
    ...
    g->vg_surface       = vkg_surface_create_for_VkhImage(g->vg_device, (void*)g->vke_image); // attachment #0 in VkFramebufferCreateInfo has 8bit samples that do not match VkRenderPass's attachment at same rank
    g->ctx              = vkg_create(g->vg_surface);
...
void vkg_path(VkgContext ctx, memory *mem) {
    type_t ty = mem->type;
    
    if (ty == typeof(r4<real>)) {
        r4<real> &m = mem->ref<r4<real>>();
        vkg_rectangle(ctx, m.x, m.y, m.w, m.h);
    }
    else if (ty == typeof(graphics::rounded::data)) {
        graphics::rounded::data &m = mem->ref<graphics::rounded::data>();
        vkg_move_to (ctx, m.tl_x.x, m.tl_x.y);
        vkg_line_to (ctx, m.tr_x.x, m.tr_x.y);
        vkg_curve_to(ctx, m.c0.x,   m.c0.y, m.c1.x, m.c1.y, m.tr_y.x, m.tr_y.y);
        vkg_line_to (ctx, m.br_y.x, m.br_y.y);
        vkg_curve_to(ctx, m.c0b.x,  m.c0b.y, m.c1b.x, m.c1b.y, m.br_x.x, m.br_x.y);
        vkg_line_to (ctx, m.bl_x.x, m.bl_x.y);
        vkg_curve_to(ctx, m.c0c.x,  m.c0c.y, m.c1c.x, m.c1c.y, m.bl_y.x, m.bl_y.y);
        vkg_line_to (ctx, m.tl_y.x, m.tl_y.y);
        vkg_curve_to(ctx, m.c0d.x,  m.c0d.y, m.c1d.x, m.c1d.y, m.tl_x.x, m.tl_x.y);
    }
        ...
        for (mx &o:m.ops) {
            ...
            if (t == typeof(graphics::movement)) {
                ...
                vkg_move_to(ctx, m.data.x, m.data.y); /// todo: convert vkvg to double.  simply not using float!
            } else if (t == typeof(vec2)) {
                vec2 l(o);
                vkg_line_to(ctx, l.data.x, l.data.y);
            }
        }
        vkg_close_path(ctx);
    }
...
text_metrics gfx::measure(str text) {
    vkg_text_extents ext;
    vkg_font_extents tm;
    ///
    vkg_text_extents(g->ctx, (symbol)text.cs(), &ext);
    vkg_font_extents(g->ctx, &tm);
...
void gfx::draw_ellipsis(str text, real w) {
    ...
    if (draw)
        vkg_show_text(g->ctx, (symbol)draw.cs());
}
...
void gfx::image(ion::image img, graphics::shape sh, vec2 align, vec2 offset, vec2 source) {
    ...
    if (!att) {
        VkgSurface surf = vkg_surface_create_from_bitmap(
    ...
    VkgSurface surf = (VkgSurface)att->data;
    draw_state   &ds = *g->ds;
    ion::size     sz = img.shape();
    r4r           &r = sh.bounds();
    assert(surf);
    vkg_set_source_rgba(g->ctx,
    ...
    scale(sc);
    /// push path
    vkg_rectangle(g->ctx, r.x, r.y, r.w, r.h);
    /// color & fill
    vkg_set_source_surface(g->ctx, surf, source.data.x, source.data.y);
    vkg_fill(g->ctx);
    pop();
}
...
void gfx::text(str text, graphics::rect rect, alignment align, vec2 offset, bool ellip) {
    ...
    ///
    vkg_save(g->ctx);
    vkg_set_source_rgba(g->ctx, c.r, c.g, c.b, c.a * ds.opacity);
    ///
    v2r            pos  = { 0, 0 };
    text_metrics   tm;
    str            txt;
    ...
    vkg_show_text(g->ctx, (symbol)text.cs());
    vkg_restore  (g->ctx);
}
...
void gfx::clear(rgba c) {
    vkg_save           (g->ctx);
    vkg_set_source_rgba(g->ctx, c->r, c->g, c->b, c->a);
    vkg_paint          (g->ctx);
    vkg_restore        (g->ctx);
}

void gfx::font(ion::font f) {
    vkg_select_font_face(g->ctx, f->alias.cs());
    vkg_set_font_size   (g->ctx, f->sz);
}

void gfx::cap  (graphics::cap   c) { vkg_set_line_cap (g->ctx, vkg_line_cap (int(c))); }
void gfx::join (graphics::join  j) { vkg_set_line_join(g->ctx, vkg_line_join(int(j))); }
void gfx::translate(vec2       tr) { vkg_translate    (g->ctx, tr->x, tr->y);  }
void gfx::scale    (vec2       sc) { vkg_scale        (g->ctx, sc->x, sc->y);  }
void gfx::scale    (real       sc) { vkg_scale        (g->ctx, sc, sc);        }
void gfx::rotate   (real     degs) { vkg_rotate       (g->ctx, radians(degs)); }
void gfx::fill(graphics::shape  p) {
    vkg_path(g->ctx, p.mem);
    vkg_fill(g->ctx);
}
...
void Vertex::calculate_tangents(Vertices& verts, mesh& input_mesh) {
    for (ngon& f: input_mesh) {
        for (u32 i = 0; i < f.size; i++) {
            ...
            ///
            vec3f deltaPos1 = verts[index2].pos - verts[index1].pos;
            vec3f deltaPos2 = verts[index3].pos - verts[index1].pos;
            vec2f deltaUV1  = verts[index2].uv  - verts[index1].uv;
            vec2f deltaUV2  = verts[index3].uv  - verts[index1].uv;

            /// set uv-weight
            float uv_weight = 1.0f / (deltaUV1.x * deltaUV2.y - deltaUV2.x * deltaUV1.y);
            
            /// calc tangent and bi-tangent
            vec3f   ta {(deltaPos1 *  deltaUV2.y - deltaPos2 * deltaUV1.y) * uv_weight},
                  bt {(deltaPos1 * -deltaUV2.x + deltaPos2 * deltaUV1.x) * uv_weight};
            
            /// normalize as n_ta and n_bt
            vec3f n_ta = ta.normalize();
            vec3f n_bt = bt.normalize();
