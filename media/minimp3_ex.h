...
#define MINIMP3_IO_SIZE (128*1024) /* mx buffer size for streaming functions, must be greater than MINIMP3_BUF_SIZE */
...
int mp3dec_detect_cb(mp3dec_mx_t *mx, uint8_t *buf, size_t buf_size)
{
    ...
    if (mx)
    {
        if (mx->seek(0, mx->seek_data))
            return MP3D_E_IOERROR;
        filled = mx->read(buf, MINIMP3_ID3_DETECT_SIZE, mx->read_data);
    ...
    if (mx)
    {
        size_t readed = mx->read(buf + MINIMP3_ID3_DETECT_SIZE, buf_size - MINIMP3_ID3_DETECT_SIZE, mx->read_data);
...
int mp3dec_load_cb(mp3dec_t *dec, mp3dec_mx_t *mx, uint8_t *buf, size_t buf_size, mp3dec_file_info_t *info, MP3D_PROGRESS_CB progress_cb, void *user_data)
{
    ...
    if (mx)
    {
        ...
        if (id3v2size)
        {
            if (mx->seek(id3v2size, mx->seek_data))
                return MP3D_E_IOERROR;
            filled = mx->read(buf, buf_size, mx->read_data);
            if (filled > buf_size)
                return MP3D_E_IOERROR;
        } else
        {
    ...
    do
    {
        ...
        if (mx)
        {
            ...
            {   /* keep minimum 10 consecutive mp3 frames (~16KB) worst case */
                ...
                filled -= consumed;
                consumed = 0;
                size_t readed = mx->read(buf + filled, buf_size - filled, mx->read_data);
...
int mp3dec_iterate_cb(mp3dec_mx_t *mx, uint8_t *buf, size_t buf_size, MP3D_ITERATE_CB callback, void *user_data)
{
    if (!mx || !buf || (size_t)-1 == buf_size || buf_size < MINIMP3_BUF_SIZE || !callback)
        return MP3D_E_PARAM;
    size_t filled = mx->read(buf, MINIMP3_ID3_DETECT_SIZE, mx->read_data), consumed = 0;
    uint64_t readed = 0;
    mp3dec_frame_info_t frame_info;
    int eof = 0;
    memset(&frame_info, 0, sizeof(frame_info));
...
seek_zero:
    ...
    {   /* no index created yet (vbr tag used to calculate track length or MP3D_DO_NOT_SCAN open flag used) */
        ...
        if (dec->mx)
        {
            if (dec->mx->seek(dec->start_offset, dec->mx->seek_data))
                return MP3D_E_IOERROR;
            int ret = mp3dec_iterate_cb(dec->mx, (uint8_t *)dec->file.buffer, dec->file.size, mp3dec_load_index, dec);
            if (ret && MP3D_E_USER != ret)
                return ret;
        } else
        {
    ...
    if (i)
    {
        ...
        if (3 == dec->info.layer)
        {
            ...
            {   /* make sure bit-reservoir is filled when we start decoding */
                ...
                {
                    hdr = dec->file.buffer;
                    if (dec->mx->seek(dec->index.frames[i - 1].offset, dec->mx->seek_data))
                        return MP3D_E_IOERROR;
                    size_t readed = dec->mx->read((uint8_t *)hdr, HDR_SIZE, dec->mx->read_data);
                    if (readed != HDR_SIZE)
                        return MP3D_E_IOERROR;
                    frame_size = hdr_frame_bytes(hdr, dec->free_format_bytes) + hdr_padding(hdr);
                    readed = dec->mx->read((uint8_t *)hdr + HDR_SIZE, frame_size - HDR_SIZE, dec->mx->read_data);
    ...
    {   /* skip not decodable first frames */
        ...
        {
            hdr = dec->file.buffer;
            if (dec->mx->seek(dec->index.frames[i].offset, dec->mx->seek_data))
                return MP3D_E_IOERROR;
            size_t readed = dec->mx->read((uint8_t *)hdr, HDR_SIZE, dec->mx->read_data);
            if (readed != HDR_SIZE)
                return MP3D_E_IOERROR;
...
do_exit:
    if (dec->mx)
    {
        if (dec->mx->seek(dec->offset, dec->mx->seek_data))
            return MP3D_E_IOERROR;
    }
    dec->buffer_samples  = 0;
    ...
    while (dec->buffer_consumed == dec->buffer_samples)
    {
        ...
        if (dec->mx)
        {
            ...
            {   /* keep minimum 10 consecutive mp3 frames (~16KB) worst case */
                ...
                dec->input_consumed = 0;
                size_t readed = dec->mx->read((uint8_t*)dec->file.buffer + dec->input_filled, dec->file.size - dec->input_filled, dec->mx->read_data);
...
int mp3dec_ex_open_cb(mp3dec_ex_t *dec, mp3dec_mx_t *mx, int flags)
{
    ...
    if (mx->seek(0, mx->seek_data))
        return MP3D_E_IOERROR;
    int ret = mp3dec_iterate_cb(mx, (uint8_t *)dec->file.buffer, dec->file.size, mp3dec_load_index, dec);
    if (ret && MP3D_E_USER != ret)
        return ret;
    if (dec->mx->seek(dec->start_offset, dec->mx->seek_data))
        return MP3D_E_IOERROR;
